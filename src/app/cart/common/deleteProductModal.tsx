    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[1000]">
      <div className="bg-white p-6 mx-2 flex flex-col gap-y-5 rounded-[12px] shadow-md text-center">
        ...
        <div className="flex justify-between w-full font-semibold text-base gap-4">
          <button
            className="border-[1px] border-[#D5D7DA] py-2.5 px-8 rounded-[12px] shrink-0"
            onClick={onClose}
          >
            Cancel
          </button>
          <button
            className="bg-red text-white border-[1px] border-red py-2.5 px-8 rounded-[12px] shrink-0"
            onClick={() => {
              onConfirm();
              onClose();
            }}
